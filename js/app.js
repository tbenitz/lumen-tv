(() => {
  ...
  const watchBlank = (video, ch, slot) => {
    ...
    let ticks = 0;
    const t = setInterval(() => {
      if (state.watchers[slot] !== id) { clearInterval(t); return; }
      const active = slot === "main" ? state.current : state.pip[slot];
      if (!active || active.url !== ch.url) { clearInterval(t); return; }
      ticks += 1;
      const frags = (video._lumen && video._lumen.frags) || 0;
      if (frags >= 1) { clearInterval(t); return; }
      if (ticks >= 5) {
        clearInterval(t);
        skipDead(ch, slot, "no .ts segments");
      }
    }, 2000);
  };
  const probeUrl = (url) => new Promise((resolve) => {
    if (!window.Hls || !Hls.isSupported()) return resolve("unknown");
    const video = document.createElement("video");
    video.muted = true;
    video.playsInline = true;
    const hls = new Hls({ enableWorker: false, maxBufferLength: 6, maxMaxBufferLength: 8 });
    let done = false;
    const finish = (result) => {
      if (done) return;
      done = true;
    ...
    hls.on(Hls.Events.FRAG_LOADED, () => finish("live"));
    hls.on(Hls.Events.ERROR, (_, data) => { if (data && data.fatal) finish("dead"); });
    hls.loadSource(url);
  ...
  async function checkFeeds(force) {
    ...
    const runOne = async (ch) => {
      ...
      checked += 1;
      if (result === "dead") { dead += 1; markDead(ch, "no .ts segments"); }
      else live += 1;
      if (checked % 2 === 0 || result === "dead") {
  ...
  function attachHls(video, url, slot) {
    ...
    video._lumen = { frags: 0, playlists: 0 };
    if (window.Hls && Hls.isSupported()) {
      const hls = new Hls({ enableWorker: true, maxBufferLength: 30 });
      let netFails = 0;
      hls.loadSource(url); hls.attachMedia(video);
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        video._lumen.playlists += 1;
        video.play().catch(() => { video.muted = true; video.play().catch(() => {}); });
      });
      hls.on(Hls.Events.LEVEL_LOADED, () => { video._lumen.playlists += 1; });
      hls.on(Hls.Events.FRAG_LOADED, () => { video._lumen.frags += 1; });
      hls.on(Hls.Events.ERROR, (_, data) => {
        if (!data.fatal) return;
        if (data.type === Hls.ErrorTypes.NETWORK_ERROR && netFails < 1) { netFails += 1; hls.startLoad(); return; }
        if (data.type === Hls.ErrorTypes.MEDIA_ERROR && netFails < 2) { netFails += 1; hls.recoverMediaError(); return; }
        const ch = slot === "main" ? state.current : state.pip[slot];
        skipDead(ch, slot, "no video segments");
      });
      state.hls[slot] = hls;
  ...
  $("helpBtn").addEventListener("click", () => { $("modalTitle").textContent = "Shortcuts"; $("customUrl").hidden = true; $("modalOk").hidden = true; $("keys").hidden = false; $("keys").textContent = "A channel is live only if HLS loads .ts segments."; modal.showModal(); });
